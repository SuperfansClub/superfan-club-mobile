...
export interface DeviceRegistration {
  id: string;
  deviceSecret: string;
  subscriptionSeverity: 'complaint' | 'severe_complaint';
}
...
export interface DeviceFilteredEscalation {
  ...
  createdAt: string;
  severity?: 'complaint' | 'severe_complaint' | 'critical';
  ratings: any;
  comments: any;
...
class DeviceService {
  ...
  async getSubscriptionSeverity(): Promise<'complaint' | 'severe_complaint' | null> {
    const registration = await this.getDeviceRegistration();
  ...
  async updateSubscriptionSeverity(newSeverity: 'complaint' | 'severe_complaint'): Promise<boolean> {
    try {
      const credentials = await this.getDeviceCredentials();
      if (!credentials) {
