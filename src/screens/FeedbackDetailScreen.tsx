...
interface ResolutionStep {
  step: number;
  description: string;
  priority: 'severe_complaint' | 'complaint';
}
...
export default function FeedbackDetailScreen({ route, navigation }: Props) {
  ...
  const generateResolutionSteps = async () => {
    try {
      // Fetch AI-powered resolution suggestions from backend
      const response = await axios.get(
        buildApiUrl(`/api/feedback/${feedback.id}/ai-resolution`)
      );
      
      if (response.data && response.data.suggestions) {
        // Convert backend suggestions to mobile app format
        const steps: ResolutionStep[] = response.data.suggestions.flatMap((suggestion: any, index: number) => 
          suggestion.actionSteps.map((step: string, stepIndex: number) => ({
            step: index * 10 + stepIndex + 1,
            description: step,
            priority: suggestion.priority || 'complaint'
          }))
        );
        
        setResolutionSteps(steps);
      } else {
        // Fallback to basic steps if API fails
        setResolutionSteps([
          {
            step: 1,
            description: "Contact customer to acknowledge their concerns and apologize for the poor experience.",
            priority: 'severe_complaint'
          },
          {
            step: 2,
            description: "Investigate the issue and implement corrective measures.",
            priority: 'complaint'
          },
          {
            step: 3,
            description: "Follow up with the customer within 24 hours to ensure satisfaction.",
            priority: 'complaint'
          }
        ]);
      }
    } catch (error) {
      console.error('Failed to fetch resolution steps from API:', error);
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            setResolving(true);
            try {
              await axios.patch(buildApiUrl(`/api/resolve-escalation/${feedback.id}`), {
      ...
      <View style={styles.section}>
        ...
        {feedback.isEscalated && (
          <View style={[styles.escalationBanner, { 
            backgroundColor: (feedback.severity === 'critical' || feedback.severity === 'severe_complaint') ? '#FEF2F2' : '#FFFBEB'
          }]}>
            <Ionicons 
              name={(feedback.severity === 'critical' || feedback.severity === 'severe_complaint') ? 'alert-circle' : 'warning'} 
              size={16} 
              color={(feedback.severity === 'critical' || feedback.severity === 'severe_complaint') ? '#EF4444' : '#F59E0B'} 
            />
            <Text style={[styles.escalationText, {
              color: (feedback.severity === 'critical' || feedback.severity === 'severe_complaint') ? '#EF4444' : '#F59E0B'
            }]}>
              {feedback.severity?.toUpperCase().replace('_', ' ')} PRIORITY: {feedback.escalationReason || 'Issue Escalated'}
            </Text>
          </View>
        )}
            ...
            {Object.entries(feedback.ratings)
              ...
              .map(([key, value], index) => (
                <View key={`${key}-${index}`} style={styles.ratingRow}>
                  ...
                  <Text style={styles.ratingRowValue}>{String(value)}/5</Text>
                </View>
              ))}
          </View>
        )}
